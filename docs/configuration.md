...
## Production Trainer (YAML + CLI)

Use the config-driven trainer for full runs with deterministic splits and exported artifacts.
...
- Reuses deterministic play splits at `outputs/splits_production.json`
- Exports TorchScript/ONNX to `outputs/exported_models/` when enabled
- Graphs cached under `cache/graphs` for faster subsequent epochs

### Key defaults from `configs/production.yaml`

- **Model:** hidden_dim=128, num_gnn_layers=6, heads=8, dropout=0.15, droppath_rate=0.08, probabilistic=True, num_modes=8, use_scene_encoder=True
- **Training:** batch_size=64, accumulate_grad_batches=2 (effective 128), max_epochs=100, min_epochs=20, num_workers=4
- **Optimization:** AdamW (lr=5e-4, weight_decay=0.05), cosine warmup (5 warmup epochs, min_lr=1e-7), SWA starts at 75% of training (swa_lrs=1e-5)
- **Regularization:** velocity=0.3, acceleration=0.1, collision=0.05, coverage=0.5 (override via CLI), Huber loss enabled with delta=1.0, label_smoothing=0.05
- **Hardware:** accelerator=gpu, devices=1, precision=bf16-mixed (RTX 40 friendly), gradient_clip_val=1.0
- **Tracking:** TensorBoard enabled by default; MLflow/W&B off in YAML
- **Data:** validate_data=False by default for speed; radius=20.0, future_seq_len=10

### CLI overrides (train_production.py)

| Argument | Default | Notes |
|----------|---------|-------|
| `--config` | `None` | Path to YAML config |
| `--mode` | `train` | `train` / `tune*` / `test*` (*placeholder today) |
| `--data-dir` | `.` | Root data directory |
| `--weeks` | all 18 | Overrides YAML weeks |
| `--hidden-dim` | 64 | Overrides model hidden dim |
| `--probabilistic` | flag | Force GMM decoder |
| `--batch-size` | 32 | Overrides YAML |
| `--learning-rate` | 1e-3 | Overrides YAML |
| `--max-epochs` | 100 | Overrides YAML |
| `--precision` | 16-mixed | Options: `32`, `16-mixed`, `bf16-mixed` |
| `--num-workers` | None | When set, overrides YAML |
| `--limit-train-batches` | None | Fraction or int for smoke tests |
| `--limit-val-batches` | None | Fraction or int for smoke tests |
| `--limit-test-batches` | None | Fraction or int for smoke tests |
| `--resume-from` | None | Resume from checkpoint path |
| `--experiment-name` | autogenerated | Names run + export files |
| `--trials` | 20 | Placeholder for future tuning |
| `--no-validation` | flag | Skip data validation |
| `--profile` | flag | Enable Lightning profiler |
| `--enable-sample-batch-warmup` | flag | Warm callbacks with a validation batch |
...
Use the provided configs as starting points:

- `configs/production.yaml` (default full run, probabilistic 8-mode, bf16)
- `configs/sanity.yaml` (fast smoke test)
- `configs/probabilistic.yaml` (legacy probabilistic profile)
- `configs/high_accuracy.yaml` (heavier deterministic model)

> The production trainer flattens YAML keys. Use field names that match `ProductionConfig` (`coverage_weight`, `accumulate_grad_batches`, etc.). Nested keys are flattened but not renamed.

Example override snippet:

```yaml
# configs/production.yaml (edit safely)
training:
  batch_size: 48
...
regularization:
  ...
  huber_delta: 1.0
loss_weights:
  coverage_weight: 0.45  # use *_weight to ensure it is picked up
hardware:
  precision: "bf16-mixed"
```

On run, the trainer writes the resolved config to `outputs/nfl_production_v2_config.json` so you can track exactly what executed.
